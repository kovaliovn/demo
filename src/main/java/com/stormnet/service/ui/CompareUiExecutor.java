...
public class CompareUiExecutor implements UiTestExecutor {
    ...
    public void execute(final Feature feature, final By... ignoredElements) {
        try {
            ...
            final Screenshot expectedScreenshot = new Screenshot(
                    ImageIO.read(new File(feature.getExpectedUiScreenshot())));
    ...
    @Override
    @Step("Take a screenshot on {feature}")
    public void execute(final SelenideElement element, final Feature feature, final By... ignoredElements) {
        try {
            final Set<By> elements = Arrays.stream(ignoredElements).collect(Collectors.toSet());
            final Screenshot actualScreenshot = new AShot()
                    .coordsProvider(new WebDriverCoordsProvider())
    ...
    private void reportDiff(final Feature feature, final ImageDiff diff, final Screenshot actualScreenshot,
            final Screenshot expectedScreenshot) throws IOException {
        final String baseName = BROWSER + "_" + feature.name().toLowerCase();
        final String diffImageName = baseName + "_diff_";
        final String actualImageName = baseName + "_actual_";
        final String expectedImageName = baseName + "_expected_";
        ImageIO.write(diff.getMarkedImage(), FORMAT_NAME, new File(TARGET + diffImageName + "." + FORMAT_NAME));
        ImageIO.write(actualScreenshot.getImage(), FORMAT_NAME,
                new File(TARGET + actualImageName + "." + FORMAT_NAME));
        ImageIO.write(
                expectedScreenshot.getImage(), FORMAT_NAME,
                new File(TARGET + expectedImageName + "." + FORMAT_NAME));
